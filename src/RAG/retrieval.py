...
def construir_indices(pasta_embeddings, pasta_saida):
    ...
    # ── LIMPEZA: remove índices antigos antes de reconstruir ──
    # Evita ficar com um banco_metadados.json "velho" misturado com um
    # FAISS/BM25 "novo" caso algo falhe no meio do processo.
    for nome_antigo in ("banco_vetorial.index", "banco_bm25.pkl", "banco_metadados.json"):
        caminho_antigo = os.path.join(pasta_saida, nome_antigo)
        if os.path.exists(caminho_antigo):
            os.remove(caminho_antigo)
            print(f"🗑️  Índice antigo removido: {nome_antigo}")
    print()
    ...
    for arquivo in arquivos:
        ...
        if not dados:
            print(f"⚠️  Aviso: {os.path.basename(arquivo)} está vazio, pulando.")
            continue

        # Checagem de dimensão: garante que todos os vetores desse arquivo
        # têm o mesmo tamanho do esperado (evita corromper o índice FAISS).
        dim_esperada = len(todos_vetores[0]) if todos_vetores else len(dados[0]['vector'])
        for item in dados:
            if len(item['vector']) != dim_esperada:
                raise ValueError(
                    f"❌ Dimensão inconsistente em {os.path.basename(arquivo)} "
                    f"(esperado {dim_esperada}, encontrado {len(item['vector'])}). "
                    f"Verifique se todos os embeddings foram gerados com o mesmo modelo."
                )
    ...
    # ── CONFERÊNCIA: lista os cursos detectados nos chunks ──────
    # Ajuda a confirmar visualmente que não sobrou nenhum nome de curso
    # "errado" (ex: com hífen) misturado no índice antes de seguir adiante.
    cursos_detectados = set()
    padrao_curso = re.compile(r'\[Curso:\s*([^\]]+)\]')
    for texto in todos_textos:
        m = padrao_curso.search(texto)
        if m:
            cursos_detectados.add(m.group(1).strip())
    print("📋 Cursos detectados nos chunks carregados:")
    for curso in sorted(cursos_detectados):
        print(f"   - {curso}")
    print()
