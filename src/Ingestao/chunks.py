...
def chunk_by_headers(text, nome_doc):
    """
    Divide o texto com base em cabeçalhos Markdown (#) ou divisões numéricas estruturadas (1.8.),
    injetando os metadados do curso em todos os blocos gerados.
    """
    chunks = []
    current_content = []
    
    # 1. PROTEÇÃO: Define um título padrão caso existam linhas antes do primeiro item do edital/PPC
    current_header = "Informações Gerais" 
    
    # Regex que captura '#' ou padrões como '1.8. ', '1.11. ', '2.1 '
    padrao_cabecalho = r'^(#+\s|\d+\.\d+\.?\s)'
    
    for line in text.split('\n'):
        if re.match(padrao_cabecalho, line):
            # Se já temos texto acumulado da seção anterior, formata e salva
            texto_acumulado = '\n'.join(current_content).strip()
            if texto_acumulado:
                # Carimba os metadados. Não aplicamos filtro de tamanho mínimo 
                # para não perder linhas ricas em dados como o Código E-mec.
                chunk_final = f"[Curso: {nome_doc}] | Seção: {current_header}\n\n{texto_acumulado}"
                chunks.append(chunk_final)
            
            # Atualiza os estados para a nova seção encontrada
            current_header = re.sub(padrao_cabecalho, '', line).strip()
            current_content = [line] # Inicia o novo bloco contendo a linha do título
            
        else:
            # Acumula todas as linhas de conteúdo, tabelas e listas que pertencem à seção atual
            current_content.append(line)
            
    # 2. GARANTIA: Processa o último bloco de texto que sobrou no fim do arquivo
    texto_acumulado = '\n'.join(current_content).strip()
    if texto_acumulado:
         chunk_final = f"[Curso: {nome_doc}] | Seção: {current_header}\n\n{texto_acumulado}"
         chunks.append(chunk_final)
...
def process_md_to_chunks(md_path, save_folder):
    ...
    base_name = os.path.splitext(os.path.basename(md_path))[0] # Ex: "Projeto-Pedagogico-SI-2026-Completo"
    
    # =================================================================
    # 🔥 A CORREÇÃO: Tradutor de Nomes de Arquivos para Nomes Reais
    # Adicione os nomes dos seus arquivos aqui e o nome oficial do curso
    # =================================================================
    mapa_cursos = {
        "Projeto-Pedagogico-SI-2026-Completo": "Sistemas de Informação",
        "PPC-CC-completo.2026-02": "Ciência da Computação",
        "PPC_EC_2025": "Engenharia de Computação",
        "Engenharia-de-Computação": "Engenharia de Computação",
        # Adicione os outros cursos da FACOM aqui seguindo o mesmo padrão...
    }
    
    # Se ele achar o nome no mapa, usa o nome bonito.
    # Se não achar, normaliza hífens/underscores para espaços como fallback seguro,
    # para nunca gravar "Engenharia-de-Computação" no metadado por engano.
    nome_oficial_curso = mapa_cursos.get(
        base_name,
        base_name.replace('-', ' ').replace('_', ' ')
    )
    
    # Passa o nome OFICIAL para a função carimbar nos blocos!
    chunks = chunk_by_headers(text, nome_doc=nome_oficial_curso) 
    ...
    print(f'✅ Processado {file_name} em {len(chunks)} chunks enriquecidos com o nome: {nome_oficial_curso}.')
